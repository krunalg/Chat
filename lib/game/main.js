...
MyGame = ig.Game.extend({
	...
	// Chat system
		...
		messages: new Array(),
		emitMsg: function(client, msg) { socket.emit('receiveMsg',client,msg); },
		updateMessageBox: function(game)
		{
	...
	update: function() {
		...
		if(ig.input.state('chat'))
		{
			...
			// while its showing and allowed to close chat
			{
				...
				{
					// if so
					this.messages.push(player.name + ': ' + $('#input').val()); // store in array
					this.emitMsg( player.name, $('#input').val() );
					this.updateMessageBox(ig.game);
					this.scrollMessageBox(ig.game);
