				...
				if(inputVal!='')
				{
					...
					if(inputVal.substr(0,1)=='/')
					{
						   ...
						   || explodeInput[0]=='/t')
						{
							...
							this.messages.push('>>' + to + ': ' + msg); // add to array for history
							this.emitTell( to, msg ); // send message to other players
							this.rebuildMessageBox(ig.game);
						}
						   ...
						   || explodeInput[0]=='/s')
						{
							...
							this.messages.push(player.name + ': ' + inputVal); // add to array for history
							this.emitSay( player.name, inputVal ); // send message to other players
							this.rebuildMessageBox(ig.game);
							//this.scrollMessageBox(ig.game); // now happens automatically
