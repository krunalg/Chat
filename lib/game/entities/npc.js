...
.defines(function(){
	...
	return url;
		...
		EntityNpc = ig.Entity.extend({
		    ...
		    moveDelay: 1000,
		    faceNextMove: function(player)
		    {
			player.facing = player.movePattern[player.moveNext];
			player.moveNext++;
			if(player.moveNext >= player.movePattern.length)
			    player.moveNext = 0; // cycle pattern
		    },
